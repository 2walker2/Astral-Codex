...
Shader "Custom/Foliage" {
    Properties {
        ...
        _EmissionStrength ("EmissionStrength", Float ) = 0.5
        _EmissionContrast ("EmissionContrast", Float ) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _EmissionContrast)
            ...
            VertexOutput vert (VertexInput v) {
                ...
                v.vertex.xyz += ((((_VertexDistortionXTexture_var.r-0.5)*node_6563.r)+(node_6563.g*sin(((o.uv0.g*_VertexDistortionScale_var.rgb.g)+(_VertexDistortionSpeed_var.rgb.g*node_2236.g)))))*normalize(v.normal));
                ...
                float facing : VFACE )
            {
                ...
                clip((_MainTex_var.a*(1.0 - step(distance(i.posWorld.rgb,_WorldSpaceCameraPos),_DepthBlendVanishTreshold_var))) - 0.5);
////// Lighting:
...
////// Specular:
                ...
                float3 node_8861 = (_MainTex_var.rgb*(_Color_var.rgb*_Brightness_var));
                float _Contrast_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Contrast );
                float3 diffuseColor = pow(node_8861,_Contrast_var); // Need this for specular when using metallic
...
////// Emissive:
                ...
                float _EmissionContrast_var = UNITY_ACCESS_INSTANCED_PROP( Props, _EmissionContrast );
                float3 emissive = pow((node_8861*_EmissionStrength_var),_EmissionContrast_var);
/// Final Color:
                outDiffuse = half4( diffuseColor, 1 );
                outSpecSmoothness = half4( specularColor, gloss );
                outNormal = half4( normalDirection * 0.5 + 0.5, 1 );
                outEmission = half4( pow((node_8861*_EmissionStrength_var),_EmissionContrast_var), 1 );
                #ifndef UNITY_HDR_ON
